...
public class AuditEventsReport implements AuditReport {
    ...
    public Page getRecords(AuditReportDto filter, Pageable pageable) {
        ...
        int fromPosition = (eventIds.size() < pageable.getOffset()) ? eventIds.size() : (int)Math.min(pageable.getOffset(), Integer.MAX_VALUE);
        int toPosition = (eventIds.size() > pageable.getOffset() + pageable.getPageSize()) ? (int)Math.min(pageable.getOffset() + pageable.getPageSize(), Integer.MAX_VALUE) : eventIds.size();
