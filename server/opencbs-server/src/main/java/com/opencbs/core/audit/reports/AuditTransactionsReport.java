...
public class AuditTransactionsReport implements AuditReport {
    ...
    public Page getRecords(AuditReportDto filter, Pageable pageable) {
        ...
        int fromPosition = (accountingEntries.size() < pageable.getOffset()) ? accountingEntries.size() : Math.toIntExact(pageable.getOffset());
        int toPosition = (accountingEntries.size() > pageable.getOffset() + pageable.getPageSize()) ? Math.toIntExact(pageable.getOffset() + pageable.getPageSize()) : accountingEntries.size();
