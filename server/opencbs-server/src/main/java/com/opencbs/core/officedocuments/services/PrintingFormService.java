...
@Slf4j
@Service
public class PrintingFormService extends AbstractDocumentService<Template>
        implements ReportService<Template, PrintingFormRequest> {
    ...
    public ResponseEntity getDocuments(PrintingFormRequest request) throws IOException {
        ...
        if (request.getFieldsValues() != null)
            fieldValues.putAll(request.getFieldsValues());
    ...
    private ResponseEntity replaceAndGetDocument(XWPFDocument xwpfDocument, Map<String, Object> data, Template template)
            throws IOException {
