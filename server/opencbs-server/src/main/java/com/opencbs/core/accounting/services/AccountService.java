...
public class AccountService extends BaseHistoryService<Account, Long, Integer> implements CrudService<Account> {
    ...
    public BigDecimal getAccountBalance(long accountId, LocalDateTime dateTime) {
        AccountEntityType account = accountEntityTypeRepository.findById(accountId).orElseThrow(() ->
                new IllegalArgumentException("Account with id:: " + accountId + " not found"));

        if (account.getType() != AccountType.BALANCE) {
...
//        }
    ...
    public Account getBalanceAccount(Account parentAccount, Account account, AccountCreatorInterface accountCreatorInterface) {
        if (parentAccount.getCloseDate() != null && parentAccount.getCloseDate().isBefore(LocalDateTime.now()))
