...
public class DayClosureProcessWorker {
    ...
    public void autoStart() {
        if( this.dayClosureProperties.getAutoStart()) {
            this.dayClosure(LocalDate.now(), UserHelper.getSystemUser());
        }
    }
    ...
    public void processDayClosure(LocalDate launchDate, User user) {
        ...
        try {
            log.info("Day closure was started from {} to {}", dayClosureDate, launchDate);
            while (dayClosureDate.isBefore(launchDate) || dayClosureDate.equals(launchDate)) {
                log.info("DayClosureProcessor day closure for date {} start", dayClosureDate);
                ...
                for (Container container : containers) {
                    ...
                    log.info("DayClosureProcessor in progress for date {}. Container {} finish, it took {} seconds",
                            dayClosureDate, container.getType().toString(), (System.currentTimeMillis() - startTimeContainer) / 1000);
                }

                dayClosureService.createDayClosure(dayClosureDate, startTime, LocalDateTime.now(),
                ...
                log.info("DayClosureProcessor day closure for date {} finish. It took {} seconds",
                        dayClosureDate, (System.currentTimeMillis() - startTimeDay) / 1000);
    ...
    private LocalDate getLastDayClosureDate(User user) {
        return dayClosureService.getLastSuccessfulDayClosureByBranch(user.getBranch())
                ...
                .orElse(LocalDate.parse(globalSettingsService.getSettingValue(DAY_CLOSURE_INIT_DATE)));
    }
